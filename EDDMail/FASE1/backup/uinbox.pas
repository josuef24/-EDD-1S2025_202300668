...
begin
  ...
  N^.Id         := AId;
  N^.Remitente  := ARem;
  N^.Estado     := AEstado;     // respeta lo que venga (p.ej. "NL")
  N^.Programado := False;       // el JSON no trae programado
  N^.Asunto     := AAsunto;
  N^.Fecha      := AFecha;      // si tu JSON no trae, puedes pasar ''
  N^.Mensaje    := AMensaje;

  N^.Prev := B.Tail;
  N^.Next := nil;

  if B.Head = nil then B.Head := N
  else B.Tail^.Next := N;
  ...
  // Mantén único el autoincremental para futuros AddMail()
