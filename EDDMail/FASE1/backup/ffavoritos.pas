...
var
  ...
  parts: TStringList;
begin
  if not Assigned(sgFav) then
  begin
    ShowMessage('Error: StringGrid no inicializado');
    Exit;
  end;
  ...
  if FavRoot = nil then
  begin
    lblTitulo.Caption := 'No hay correos favoritos';
    Exit;
  end;
  ...
  try
    ...
    begin
      ...
      try
        ...
        lblTitulo.Caption := Format('Total de correos favoritos: %d', [FItems.Count]);
      finally
        parts.Free;
      end;
    end
