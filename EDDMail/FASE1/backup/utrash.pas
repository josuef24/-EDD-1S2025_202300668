...
type
  ...
  TTrash = record
    ...
    Count: Integer;    // [DATA] Llevo el conteo de elementos por conveniencia
  end;
...
begin
  // [EDIT] Si no me pasan un correo válido, no hago nada
  if M = nil then Exit;

  // [MEM] Reservo memoria para el nuevo nodo y lo apilo al tope
  ...
  Inc(T.Count);    // [DATA] Actualizo el tamaño de la pila
end;
...
begin
  // [CHECK] Si está vacía, retorno nil
  if T.Top = nil then Exit(nil);

  // [POP] Quito el tope y regreso el correo que contenía
  ...
  // [MEM] Libero el nodo de la pila (el correo no lo libero aquí)
  Dispose(N);
end;

function PeekTrash(const T: TTrash): PMail;
begin
  // [LECTURA] Solo miro el tope sin desapilar; si no hay, devuelvo nil
  if T.Top <> nil then Result := T.Top^.Mail else Result := nil; // [APUNTADOR] Acceso al campo del nodo
end;

function IsTrashEmpty(const T: TTrash): Boolean;
begin
  // [CHECK] La pila está vacía si Top es nil
  Result := T.Top = nil;
end;
